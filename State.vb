Public Class State
            ...
            Select Case m_ToolSelected
                Case Tools.Pointer
                    ...
                    If Not m_bMouseDown Then
                        Dim a As Integer = Math.Abs(m_pLocation.X - e.Location.X)
                        Dim b As Integer = Math.Abs(m_pLocation.Y - e.Location.Y)
                        Dim hypotenuse As Integer = Math.Sqrt((a ^ 2) + (b ^ 2))
                        Dim radius As Integer = CircleDiam / 2
        ...
        Select Case m_ToolSelected
            Case Tools.Pointer
                If m_bMouseEntered Then
                    If Not m_bMouseDown Then
                        Dim pbx As PictureBox = CType(sender, PictureBox)
                        m_pDiff = New Point(e.Location.X - (m_pLocation.X - CInt(m_bmpCircle.Width / 2)), e.Location.Y - (m_pLocation.Y - CInt(m_bmpCircle.Height / 2)))
    ...
    Private Sub MoveState(ByVal pbx As PictureBox, ByVal p As Point)
        ...
        Dim g As Graphics = Graphics.FromImage(bmp)
        Dim pLocation As New Point(p.X - m_pDiff.X, p.Y - m_pDiff.Y)
        m_pLocation = New Point(p.X - m_pDiff.X + CInt(m_bmpCircle.Width / 2), p.Y - m_pDiff.Y + CInt(m_bmpCircle.Height / 2))
        g.DrawImage(m_bmpCircle, pLocation)
    ...
    Private Sub HighlightState(ByVal pbx As PictureBox, ByVal p As Point)
        Dim bmp As Bitmap = BackImage(pbx)
        Dim g As Graphics = Graphics.FromImage(bmp)
        Dim pLocation As New Point(CInt(p.X - (m_bmpCircle.Width / 2)), CInt(p.Y - (m_bmpCircle.Height / 2)))
        g.DrawImage(m_bmpHighlightedCircle, pLocation)
        pbx.Image = bmp.Clone
        bmp.Dispose()
        g.Dispose()
    End Sub
